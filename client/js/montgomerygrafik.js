Template.montgomerygrafik1.helpers({
	selected: function() {
		return Session.equals("selectedpatient", this.__originalId) ? "selected" : '';
	}
});
Template.montgomerygrafik.helpers({
...
Template.montgomerygrafik.onCreated(function() {
...
Template.montgomerygrafik.rendered = function(){
	Tracker.autorun(function () {
		if (chart1.ready()) {
			...
			var datavalues=budgetdata.montgomerymeasurescore;
			var datalabels=budgetdata.montgomerytreatmenttime;
			console.log(datavalues);
			drawchart(datavalues,datalabels);
		}
	});
};
Template.montgomerygrafik.events({
